...
import {
  ApolloPreviewModal,
  fetchDataFromApolloWithDomainFilter,
} from "../ApolloPreviewModal";
import {
  fetchJobName,
  getDomainData,
  saveProximityJob,
  updateProximityJob,
} from "./ProximityHelper";
import { FieldSelecter } from "./FieldSelector";
import { PreviewDomain } from "./PreviewDomain";
import {
  reverseTransformStoreleadsFilters,
  verifyEmailIds,
} from "../../../utils/HelperFunctions";
import { useMainScreenContext } from "../../../sharedComponents/contexts/MainScreenContext";
import { CreateProximityContextProvider,useCreateProximityProvider } from "../../../sharedComponents/contexts/CreateProximityContext";

const CreateProximity = ({ editingMode }) => {
	
  const {setEditingMode} = useCreateProximityProvider()
  setEditingMode(editingMode)

  const [domainName, bindDomainName, resetDomainName, setDomainName] =
    useInput("");
  const [keywordInput, bindKeywordInput, resetKeywordInput, setKeywordInput] =
    useInput("");
  ...
  const { doRefresh } = useMainScreenContext();
  ...
  const isUpdatePossible = () => {
    return labelEmailIdVerify?.hint == ReactiveLabelEnum.success;
  };

  const saveData = () => {
    setLabelSaveMessage({ hint: "", message: "Creating Job..." });
    saveProximityJob(domainName, jobName, emailIds, personaList, {
  ...
  const updateData = () => {
    ...
    updateProximityJob(jobData._id, emailIds, personaList, {
      ...selectedDomainAttributes,
      keyword: keywordInput,
    })
      .then(() => {
  ...
  return (
      <div className="d-flex flex-column  align-items-center h-100">
            ...
            <>
              {!!domainData && (
                <FieldSelecter
                  ...
                  proximityFieldValues={selectedDomainAttributes}
                  setProximityFieldValues={setSelectedDomainAttributes}
                />
              )}
          ...
          {isSavingFilter && (
            <>
              ...
              <div className="my-3 d-flex justify-content-center">
                <ApolloPreviewModal
                  dataFunction={(filter) => {
                    return fetchDataFromApolloWithDomainFilter(
                      { ...selectedDomainAttributes, keyword: keywordInput },
                      filter
                    );
                  }}
                  ...
                  isEnabled={
                    !!selectedDomainAttributes &&
                    Object.keys(selectedDomainAttributes).length > 0
                  }
                />
              </div>
        ...
        <PreviewDomain
          showPreviewModal={previewDomainModal}
          setShowPreviewModal={showPreviewDomainModal}
          jobData={{ ...selectedDomainAttributes, keyword: keywordInput }}
        />
      </div>
  );
};
...
export default ({editingMode})=>(
	<CreateProximityContextProvider>
		<CreateProximity editingMode={editingMode} />
	</CreateProximityContextProvider>
)
